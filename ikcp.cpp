...
const IUINT32 IKCP_CMD_UDP_PUSH = 85;	// cmd: push udp data
const IUINT32 IKCP_ASK_SEND = 1;		// need to send IKCP_CMD_WASK
const IUINT32 IKCP_ASK_TELL = 2;		// need to send IKCP_CMD_WINS
const IUINT32 IKCP_WND_SND = 32;
const IUINT32 IKCP_WND_RCV = 32;
const IUINT32 IKCP_MTU_DEF = 996;
...
void ikcp_release(ikcpcb *kcp)
{
	...
	if (kcp) {
		...
		while (!iqueue_is_empty(&kcp->rcv_udp_queue)) {
			seg = iqueue_entry(kcp->rcv_udp_queue.next, IKCPSEG, node);
			iqueue_del(&seg->node);
			ikcp_segment_delete(kcp, seg);
		}
		if (kcp->buffer) {
...
int ikcp_udp_recv(ikcpcb *kcp, char *buffer, int len)
{
	struct IQUEUEHEAD *p;
	...
	int sz = 0;
	if (!iqueue_is_empty(&kcp->rcv_udp_queue)) {
		for (p = kcp->rcv_udp_queue.next; p != &kcp->rcv_udp_queue; ) {
			seg = iqueue_entry(p, IKCPSEG, node);
			if (sz + seg->len >(unsigned int)len)
				break;
			p = p->next;
			if (buffer) {
			...
			if (ikcp_canlog(kcp, IKCP_LOG_RECV)) {
				ikcp_log(kcp, IKCP_LOG_RECV, "udp recv sn=%lu", seg->sn);
			}
...
//---------------------------------------------------------------------
int ikcp_recv_ex(ikcpcb *kcp, char *buffer, int len)
{
	struct IQUEUEHEAD *p;
	int ispeek = (len < 0) ? 1 : 0;
	int peeksize;
...
int ikcp_send(ikcpcb *kcp, const char *buffer, int len)
{
	...
	if (kcp->stream != 0) {
		if (!iqueue_is_empty(&kcp->snd_queue)) {
			...
			if (old->len < kcp->mss && old->not_reliable != 1) {
...
// user/upper level send, returns below zero for error
//---------------------------------------------------------------------
int ikcp_udp_send(ikcpcb *kcp, const char *buffer, int len)
{
	IKCPSEG *seg;
...
int ikcp_input(ikcpcb *kcp, const char *data, long size)
{
	...
	while (1) {
		...
		if (cmd == IKCP_CMD_ACK) {
			...
			iqueue_init(&seg->node);
			iqueue_add_tail(&seg->node, &kcp->rcv_udp_queue);
			kcp->nrcv_udp_que++;
		}
...
void ikcp_flush(ikcpcb *kcp)
{
	...
	// 3time for fail 
	bool slow_cwd = false;
	static unsigned int slow_xmit_cnt = 3;
	...
	while (_itimediff(kcp->snd_nxt, kcp->snd_una + cwnd) < 0) {
		...
		if (newseg->not_reliable == 1) {
			newseg->cmd = IKCP_CMD_UDP_PUSH;
	...
	for (p = kcp->snd_buf.next; p != &kcp->snd_buf; p = next) {
		...
		if (segment->xmit == 0) {
			...
			// statistic
			kcp->send_data_size += segment->len;
		}
			...
			// lost = 1;
			if (segment->xmit % slow_xmit_cnt == 0) {
				slow_cwd = true;
			}
			// statistic
			kcp->t_resnd_size += (IKCP_OVERHEAD + segment->len);
			kcp->t_resnd_times++;
		}
